...
int query_max(int vertex, int start, int end, int l, int r) {
  // printf("vertex=%2d, query [%2d, %2d] @ [%2d, %2d]
  ...
  int result;
  if (r <= mid)
    return query_max(vertex * 2, start, mid, l, r);
  if (l > mid)
    return query_max(vertex * 2 + 1, mid + 1, end, l, r);
  return max(query_max(vertex * 2, start, mid, l, r),
             query_max(vertex * 2 + 1, mid + 1, end, l, r));
}
